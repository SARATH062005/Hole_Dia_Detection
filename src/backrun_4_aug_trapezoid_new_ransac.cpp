...
)
{
    ...
    points[0].x = center.x + start_radius * std::cos(angle_rad - delta_rad);
    points[0].y = center.y + start_radius * std::sin(angle_rad - delta_rad);

    points[1].x = center.x + start_radius * std::cos(angle_rad + delta_rad);
    points[1].y = center.y + start_radius * std::sin(angle_rad + delta_rad);

    // Outer arc points (end_radius, at angle ± delta)
    points[2].x = center.x + end_radius * std::cos(angle_rad + delta_rad);
    points[2].y = center.y + end_radius * std::sin(angle_rad + delta_rad);

    points[3].x = center.x + end_radius * std::cos(angle_rad - delta_rad);
    points[3].y = center.y + end_radius * std::sin(angle_rad - delta_rad);
    ...
    int thickness = 2)
{
    ...
    trapezoid_points[0] = cv::Point2f(center.x + start_radius * std::cos(angle_rad - delta_rad), center.y + start_radius * std::sin(angle_rad - delta_rad));
    trapezoid_points[1] = cv::Point2f(center.x + start_radius * std::cos(angle_rad + delta_rad), center.y + start_radius * std::sin(angle_rad + delta_rad));
    trapezoid_points[2] = cv::Point2f(center.x + end_radius * std::cos(angle_rad + delta_rad), center.y + end_radius * std::sin(angle_rad + delta_rad));
    trapezoid_points[3] = cv::Point2f(center.x + end_radius * std::cos(angle_rad - delta_rad), center.y + end_radius * std::sin(angle_rad - delta_rad));
...
{
    namespace fs = std::filesystem;
    std::string parentFolder = "/mnt/4tb/home/griffyn/Music/images/POST_ANO_OPT_trials/RAW_AUG_6_2PM_24_Cycles";
    std::string csvPath = "/mnt/4tb/home/griffyn/Music/images/POST_ANO_OPT_trials/RAW_AUG_6_2PM_24_Cycles/RAW_AUG_6_2PM_24_Cycles.csv";
    // Aug_4_exposures
    const std::map<std::string, int> Diameter_hole_expo = {
        {"RS4", 45000}, {"LA3", 17677}, {"LS4", 35000}, {"LA6", 17677}, {"UA1", 45000}, {"RC2", 8000}, {"RC1", 36108}, {"RC3", 8000}, {"ML2", 5000}, {"LA2", 6000}, {"SP5", 5000}, {"SP1", 18000}, {"SP9", 10000}, {"LA4", 4188}, {"LA5", 4188}, {"UA6", 17000}};
    // Aug_2_exposures
    // const std::map<std::string, int> Diameter_hole_expo = {
